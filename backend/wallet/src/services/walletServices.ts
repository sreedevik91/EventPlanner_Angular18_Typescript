import { IEmailService, IResponse, IRequestParams, IServiceGrpcType, SERVICE_RESPONSES, IWalletRepository, IWallet, IWalletService, IWalletTransactions } from "../interfaces/walletInterfaces"
...
export class WalletService implements IWalletService {
    ...
    async getWalletById(walletId: string) {
        ...
        try {
            const walletData = await this.walletRepository.getWalletById(walletId)

            console.log('getWalletById service response: ', walletData);

            return walletData ? { success: true, data: walletData } : { success: false, message: SERVICE_RESPONSES.fetchDataError }
    ...
    async getWalletByUserId(userId: string) {
        ...
        try {
            const userWalletData = await this.walletRepository.getWalletByUserId(userId)
            console.log('getWalletByUserId service response: ', userWalletData);

            if (!userWalletData) {
                const newWallet = await this.walletRepository.createWallet({ userId, amount: 0, transactions: [] })
                return newWallet ? { success: true, data: newWallet } : { success: false, message: SERVICE_RESPONSES.fetchDataError }
            }

            return userWalletData ? { success: true, data: userWalletData } : { success: false, message: SERVICE_RESPONSES.fetchDataError }
    ...
    async deleteWalletById(walletId: string) {
        ...
        try {
            const deleteWallet = await this.walletRepository.deleteWallet(walletId)
            console.log('deleteWalletById service response: ', deleteWallet);

            return deleteWallet ? { success: true, data: deleteWallet, message: SERVICE_RESPONSES.deleteWalletSuccess } : { success: false, message: SERVICE_RESPONSES.deleteWalletError }
    ...
    async updateWalletById(walletId: string, data: Partial<IWalletTransactions>) {

        try {
            const { amount, type } = data
            const userWallet = await this.walletRepository.getWalletById(walletId)
            let newAmount = 0
            if (userWallet) {
            ...
            const updateWallet = await this.walletRepository.updateWallet(walletId, { $set: { amount: newAmount }, $push: {transactions:transaction} })
            console.log('updateWalletById service response: ', updateWallet);

            return updateWallet ? { success: true, data: updateWallet, message: SERVICE_RESPONSES.updateWalletSuccess } : { success: false, message: SERVICE_RESPONSES.updateWalletError }
    ...
    async createWallet(data: Partial<IWallet>) {

        try {
            const newWallet = await this.walletRepository.createWallet(data)
            console.log('createWallet service response: ', newWallet);

            return newWallet ? { success: true, data: newWallet, message: SERVICE_RESPONSES.addWalletSuccess } : { success: false, message: SERVICE_RESPONSES.addWalletError }
    ...
    //         const { user, walletId, serviceId, providerId, event, style, services, deliveryDate, venue, totalCount } = bookingData
    ...
    // async getBookingBywalletId(id: string) {
    //     try {
    //         const booking = await this.bookingRepository.getBookingBywalletId(id)
    ...
    //     } catch (error: unknown) {
    //         error instanceof Error ? console.log('Error message from getBookingBywalletId service: ', error.message) : console.log('Unknown error from getBookingBywalletId service: ', error)
    ...
    //             const userData = await getUserByIdGrpc(updateBooking?.walletId!)
    //             let content = `
